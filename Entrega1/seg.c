...
int regla5(int com, Queue* *colas, int periodo_tiempo, int n_queues, int time);
//////////////////////////////////////////////////////main//////////////////////////////////////////////////////////////////
Array_list* array;
	...
	if(proceso_en_cpu==NULL){
		...
		while(prioridad>0 && proceso_en_cpu==NULL){
			if(colas[prioridad]->head!=NULL){
				...
				printf("TIME:%i ha ingresado a cpu : %s  \n",time,proceso_en_cpu->name);
		...
		if(time % periodo_tiempo == 0){
			...
			if(n_queues > 1){
				...
				for( i = n_queues-1; i > 0; i--){
					...
					//printf("extending %i ",colas[i]->cantidad_de_procesos);
					extend(colas[n_queues], colas[i]); // suben a la de mayor prioridad

				}
				return 1;

				//printf("cantidad de procesos: %i\n",colas[n_queues]->cantidad_de_procesos);
			}
		}
	}
	return 0;
